...
def tabel_daftar_pengguna(request):
    if not is_authenticated(request):
        return redirect("/auth/login?next=/auth/daftar-pengguna")
    ...
    data = query(
        f"""
    SELECT p.email, p.nama, p.statusverifikasi, i.nik
    FROM penggalang_dana p left outer join individu i on p.email=i.email"""
    )
    context = {"pengguna": data}
    print(data)
    return render(request, "tabel_daftar_pengguna.html", context)
...
def admin_pov_read_pengguna(request):
    ...
    data = query(
        f"""
    SELECT p.email, p.nama, p.nomorhp, p.alamat, p.namabank, p.norek, i.nik, i.tanggallahir, i.jeniskelamin, p.linkrepo, o.namaorganisasi, o.nap, o.notelporganisasi, o.tahunberdiri
    FROM penggalang_dana p left outer join individu i on p.email=i.email
    left outer join organisasi o on p.email=o.email
    WHERE p.email='{email}'"""
    )
    context = {"p": data[0]}
    return render(request, "admin_pov_read_pengguna.html", context)
...
def verifikasi_pengguna(request):
    ...
    result = query(
        f"""
        UPDATE penggalang_dana
        SET statusverifikasi = 'Sudah Terverifikasi'
        WHERE email='{email}'"""
    )

    return redirect("/auth/daftar-pengguna")
